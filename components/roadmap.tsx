    ...
    <div ref={ref} className="relative w-full h-full">
      ...
      >
        <div className="bg-white/5 backdrop-blur-sm rounded-xl border border-white/10 p-4 sm:p-5 lg:p-6 xl:p-7 2xl:p-8 shadow-md flex flex-col h-full w-full min-h-[280px] sm:min-h-[320px] lg:min-h-[360px] xl:min-h-[380px] 2xl:min-h-[400px]">
          <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs sm:text-sm font-bold uppercase tracking-wider mb-3 sm:mb-4 bg-gradient-to-r ${item.gradient} text-white flex-shrink-0`}>
          ...
          <p className="text-sm sm:text-base lg:text-lg font-medium mb-4 sm:mb-5 leading-relaxed flex-shrink-0" style={{ color: item.color }}>
            {item.description}
          </p>
...
export default function Roadmap() {
  ...
  return (
    <section
      id="roadmap"
      className="py-16 md:py-24 lg:py-32 relative overflow-hidden overflow-x-hidden"
        ...
        <div className="hidden sm:block">
          <div className="hidden 2xl:grid grid-cols-4 gap-8 items-start mb-8">
            {roadmapItems.slice(0, 4).map((item, index) => (
              <TimelineItem key={index} item={item} index={index} activeItem={activeItem} hoveredItem={hoveredItem} setHoveredItem={setHoveredItem} />
            ))}
          </div>
          <div className="hidden 2xl:grid grid-cols-3 gap-8 items-start justify-center">
            {roadmapItems.slice(4).map((item, index) => (
              <TimelineItem key={index + 4} item={item} index={index + 4} activeItem={activeItem} hoveredItem={hoveredItem} setHoveredItem={setHoveredItem} />
            ))}
          </div>
          ...
          <div className="hidden xl:grid 2xl:hidden grid-cols-3 gap-6 items-start mb-6">
            {roadmapItems.slice(3, 6).map((item, index) => (
              <TimelineItem key={index + 3} item={item} index={index + 3} activeItem={activeItem} hoveredItem={hoveredItem} setHoveredItem={setHoveredItem} />
            ))}
          </div>
          <div className="hidden xl:flex 2xl:hidden justify-center">
            <div className="w-full max-w-sm">
              <TimelineItem item={roadmapItems[6]} index={6} activeItem={activeItem} hoveredItem={hoveredItem} setHoveredItem={setHoveredItem} />
            </div>
          </div>
